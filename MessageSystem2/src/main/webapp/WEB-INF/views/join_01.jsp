...
<body>
	<ul class="actions vertical">
		...
		<form action="memberInsert" onsubmit="return validateForm()">
			<li><input type="text" placeholder="숫자만 입력해주세요" name="phone"
				id="phone"></li>

			<li><input type="password" placeholder="PW를 입력하세요" id="pw"
				name="pw"></li>
			<li><input type="password" placeholder="PW확인" id="pwCheck"
				name="pwCheck">
			<h4 id="passwordMessage"></h4></li>

			<li><input type="button" value="비밀번호 확인"
				onclick="checkPasswordMatch()"></li>

			<li><input type="text" placeholder="이름을 입력하세요" name="name"
				id="name"></li>

			<li><label for="email">이메일:</label> <input type="text"
				name="email" id="email" placeholder="이메일을 입력하세요"> <select
				name="email_domain" id="email_domain" onchange="updateEmail()">
			...
			</select> <input type="text" name="custom_email" id="custom_email"
				style="display: none;" placeholder="직접입력"></li>
			...
			<li><input type="submit" id="joinUsButton" value="JoinUs"
				class="button fit" disabled></li>
		</form>
	</ul>
	<script>
		...
		function checkAllFieldsFilled() {
			// 필수 입력 필드 배열
			var requiredFields = [ 'email', 'pw', 'pwCheck', 'address', 'name',
					'phone', 'birth' ];
